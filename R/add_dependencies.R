...
add_dependencies <- function(compendium = NULL) {
  ...
  duplicated_deps <- deps_in_functions[which(
    deps_in_functions %in%
      deps_in_namespace
  )]
  ...
  if (length(duplicated_deps)) {
    ...
    ui_info(paste0(
      "The following function(s) are already present in ",
      "{ui_value('NAMESPACE')}:"
    ))
  ...
  if (length(deps_in_package)) {
    deps_in_package <- unlist(lapply(
      strsplit(deps_in_package, "::"),
      function(x) x[1]
    ))
  }
  ...
  if (length(deps_suggest)) {
    deps_suggest <- unlist(lapply(strsplit(deps_suggest, "::"), function(x) {
      x[1]
    }))
  }
  ...
  pkgs_in_imports <- sort(deps_in_package[
    !(deps_in_package %in%
      pkgs_in_depends)
  ])
  ...
  if (length(pkgs_in_imports)) {
    ...
    if (length(pkgs_with_version)) {
      pkgs <- unlist(lapply(strsplit(pkgs_with_version, "\\s\\("), function(x) {
        x[1]
      }))
    ...
    ui_line(paste0(
      "  {cli::symbol$radio_on} Adding the following ",
