...
#'   each push. See [add_github_action()] for further information.
#'
#'   If `create_repo = FALSE` this argument is ignored.
#'
...
create_new_package <- function(
  ...
  if (is.null(given)) {
    ...
    if (is.null(given)) {
      stop(
        "Please provide a given name. Use `set_credentials()` to ",
  ...
  if (is.null(family)) {
    ...
    if (is.null(family)) {
      stop(
        "Please provide a family name. Use `set_credentials()` to ",
  ...
  if (is.null(email)) {
    ...
    if (is.null(email)) {
      stop(
        "Please provide an email address. Use `set_credentials()` to ",
  ...
  if (create_repo) {
    if (gh::gh_token() == "") {
      stop(
        "No GITHUB PAT found. Please run `usethis::gh_token_help()` for ",
  ...
  if (!is.null(status)) {
    if (
      ...
      stop(
        "Invalid Repo status. Please run `?add_repostatus_badge` to ",
  ...
  if (!is.null(lifecycle)) {
    if (
      !(tolower(lifecycle) %in%
        c("experimental", "stable", "deprecated", "superseded"))
    ) {
      stop(
        "Invalid Life cycle. Please run `?add_lifecycle_badge` to ",
        "select an appropriate one."
      )
    }
  }
  ...
  if (!quiet) {
    ui_done("Writing {ui_value('R/fun-demo.R')} file")
  }
  ...
  if (!quiet) {
    ui_done("Kniting {ui_value('README.Rmd')}")
  }
  ...
  if (!quiet) {
    ui_done(paste0(
      "Committing changes with the following message: ",
      "{ui_value('init repo')}"
    ))
  }
  ...
  if (
    gh_check || codecov || gh_render || website || gh_citation || gh_codemeta
  ) {
    ui_title("Committing changes")
