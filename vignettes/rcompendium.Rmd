...
directly with the functions `create_new_*()` and `add_*()`. But if you create 
...
3. Run `create_package()` to create a new package structure or `create_new_compendium()` 
...
In addition to these three setup functions (`set_credentials()`, `create_new_package()`, 
`create_new_compendium()`), the package `rcompendium` offers 29 other functions.
